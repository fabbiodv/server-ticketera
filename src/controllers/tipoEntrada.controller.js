...
export const getTiposEntrada = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error al obtener tipos de entrada: " + error.message });
  }
};
...
export const getTipoEntradaById = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Error al obtener tipo de entrada: " + error.message });
  }
};
export const getTiposEntradaByEventoId = async (req, res) => {
  try {
    ...
    if (!tiposEntrada || tiposEntrada.length === 0) {
      return res
        .status(404)
        .json({ error: "No se encontraron tipos de entrada para este evento" });
    }
    ...
    res
      .status(500)
      .json({ error: "Error al obtener tipos de entrada por evento" });
  }
};
export const createTipoEntrada = async (req, res) => {
  try {
    const {
      ...
      eventoId,
      totalEntradas,
      maximoEntradasPorPersona,
    } = req.body;
    if (!nombre || !precio || !eventoId || !maximoEntradasPorPersona) {
      return res.status(400).json({
        error:
          "Todos los campos son obligatorios: nombre, precio, eventoId, maximoEntradasPorPersona",
      });
    }
